...
- Theme colours (background/platform/square)
...
This is directly inspired by your original square platformer's responsibilities:
- parse JSON
- map platforms array
- apply theme + physics
- infer canvas size
...
class WorldLevel {
  constructor(levelJson) {
    ...
    this.theme = Object.assign(
      { bg: "#F0F0F0", platform: "#246324", square: "#132F8F" },
      levelJson.theme || {},
    );

    // Physics knobs (the square player will read these).
  ...
  */
  drawWorld() {
    ...
    for (const p of this.platforms) {
      const platformColor = p.color ? color(p.color) : color(this.theme.platform);
      p.draw(platformColor);
    }
  }
}
